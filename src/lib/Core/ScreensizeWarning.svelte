...
<dialog class="modal modal-bottom sm:modal-middle bg-base-100" {open}>
	<div class="modal-box bg-error text-error-content">
		...
		<p class="pt-4">
			...
			completely reset WidgetHub (nuclear). You also have the option to resize the boxsize; however,
			please note that this action <b>WILL</b> disrupt components, as it <b>does not</b> adjust widgets
			to meet their minimum size requirements.
		</p>
		<div class="modal-action">
			<div class="join">
				<button class="btn join-item" onclick={reset}>Reset</button>
				<button class="btn btn-primary join-item" onclick={resize}>Resize</button>
			</div>
		</div>
	</div>
</dialog>
