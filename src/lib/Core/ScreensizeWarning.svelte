...
<dialog class="modal modal-bottom sm:modal-middle bg-base-100" {open}>
  <div class="modal-box bg-error text-error-content">
    ...
    <p class="py-4">Resize the window to make it larger or press the reset button to reset your profile or press Resize to recalculate .</p>
    <div class="modal-action">
      <div class="join">
        <button class="btn join-item" onclick={_ => {localStorage.clear(); location.reload();}}>Reset</button>
        <button class="btn btn-primary join-item" onclick={_ => profiles.profile.gridSize.boxSize = calcBoxSize(innerWidth, innerHeight, gridSize.rows, gridSize.cols)}>Resize</button>
      </div>
    </div>
  </div>
</dialog>
